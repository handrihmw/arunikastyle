    ...
    <section id="summary-cart-wrapper">
        <div class="as-card-base as-card-base--full-width summary-cart">
            ...
            <div>
                        ...
                        <div v-if="tmpCart.length">
                            <div v-for="item in tmpCart" :key="item.id">
                                <div v-if="item.action != 'owndomain'">
                                    <div class="row mb-2">
                                        <div class="col-10">
                                            <div class="summary-cart__item-type">
                                                {{ itemType(item) }}
                                                <div class="as-card-base__feature--icon as-card-base__feature--icon-exclamation as-tooltip">
                                                    <p class="as-tooltip__info as-tooltip__info--bottom"  :class="{'as-tooltip__domain':item.type == 'domain'}">Besar biaya perpanjangan Anda adalah Rp {{ item.full_price | formatPrice }}</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-2">
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            <div v-if="item.type == 'domain' && !item.cart_type && item.cart_type == 'tmp_domain'" class="summary-cart__item-title">
                                                {{ item.register_sld + item.register_tld}}
                                            </div>
                                            <div v-if="item.type == 'hosting' && flow === 'website'" class="summary-cart__item-title">{{ item.period_text }}</div>
                                            <div v-else class="summary-cart__item-title">
                                                <div v-if="item.type == 'hosting' && item.discount_enabled == 1">
                                                    Diskon {{ discountPrice(item.total, item.full_price) }}%
                                                </div>
                                                <span v-if="item.type == 'domain'">Gratis for </span>
                                                <span v-else-if="item.type == 'hosting'">Hosting </span>
                                                {{ item.title }}
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div id="hosting-package-price" class="summary-cart__item-price text-right">
                                                <div>Rp {{ item.total | formatPrice }}</div>
                                                <s v-if="item.discount_enabled == 1 || item.total == 0" class="summary-cart__normal-price">Rp {{ item.full_price | formatPrice }}</s>
                                            </div>
                                            <div v-if="plan_type === 'cpanel' && item.type === 'cpanel_license'" class="summary-cart__item-price">
                                                <s>Rp {{ item.discount_price | formatPrice }}</s>
                                            </div>
                                        </div>
                                    </div>
                                    <hr>
                    ...
                    <transition name="fade" mode="out-in">
                        <div v-if="orderflow_step == 1 &&
                            ...
                            <div class="row justify-content-left">
                                <div class="col-8">
                                    <div class="summary-cart__item-type" v-if="flow === 'website'">Website {{ templateName }}</div>
                                    <div class="summary-cart__item-type" v-else-if="flow === 'mailhosting'">Mail Hosting {{ period[hosting.selected.period].text }}</div>
                                    <div class="summary-cart__item-type" v-else-if="flow === 'vps' && plan_type === 'cpanel'">VPS cPanel {{ period[vps.selected.period].text }}</div>
                                    <div class="summary-cart__item-type" v-else-if="flow === 'vps'">Cloud VPS {{ period[vps.selected.period].text }}</div>
                                    <div class="summary-cart__item-type" v-else-if="flow === 'vps_kvm'">VPS KVM {{ period[vps_kvm.selected.period].text }}</div>
                                    <div class="summary-cart__item-type" v-else>
                                        Hosting {{ period[hosting.selected.period].text }} 
                                        <div class="as-card-base__feature--icon as-card-base__feature--icon-exclamation as-tooltip">
                                            <p class="as-tooltip__info as-tooltip__info--bottom">Besar biaya perpanjangan Anda adalah Rp {{ hosting.plan_period[hosting.selected.period].price | formatPrice }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-6">
                                    ...
                                    <div class="summary-cart__item-title" v-else>
                                        <div v-if="hosting.plan_period[hosting.selected.period].discount_enabled == 1">
                                            Diskon {{ discountPrice(hosting.plan_period[hosting.selected.period].discount_price, hosting.plan_period[hosting.selected.period].price) }}%
                                        </div>
                                        <div>Hosting {{ hosting.selected.plan.title }}</div>
                                    </div>
                                </div>
                                <div class="col-6">             
                                    <div v-if="flow != 'vps' && flow != 'vps_kvm'">
                                        <div v-if="flow === 'website' && hosting.plan_period[hosting.selected.period].discount_enabled == 1" id="hosting-package-price" class="summary-cart__item-price">Rp {{ hosting.plan_period[hosting.selected.period].discount_price | formatPrice }}</div>
                                        <div v-else-if="flow === 'website'" id="hosting-package-price" class="summary-cart__item-price">Rp {{ hosting.plan_period[hosting.selected.period].price | formatPrice }}</div>
                                        <div v-else-if="hosting.plan_period[hosting.selected.period].discount_enabled == 1" class="summary-cart__item-price">
                                            <div>Rp {{ hosting.plan_period[hosting.selected.period].discount_price | formatPrice }}</div>
                                            <s class="summary-cart__normal-price">Rp {{ hosting.plan_period[hosting.selected.period].price | formatPrice }}</s>
                                        </div>
                                        <div v-else id="hosting-package-price" class="summary-cart__item-price">Rp {{ hosting.plan_period[hosting.selected.period].price | formatPrice }}</div>
                                    </div>
                ...
                <hr>
                <div class="row as-mb-8" v-if="coupon_code">
                    <div class="col-10">
                        <div class="summary-cart__item-coupon">
                            <div>Kupon <span>{{ coupon_code }}</span> telah diterapkan.</div>
                        </div>
                    </div>
                    <div class="col-2">
                ...
                <div class="row as-mb-10" v-if="tmpDiscount != 0">
                    <div class="col-12">
                        <div class="summary-cart__item-save">
                            <div>Selamat! Anda berhasil berhemat <span>Rp {{ Number(tmpDiscount) | formatPrice }}</span>.</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-9">
                            <p class="summary-cart__total-price">
                                Rp {{ (Number(tmp_cart.total) + Number(tmp_domain_suggestion.total) + Number(tmp_additional.total)) + ((Number(tmp_cart.total) + Number(tmp_domain_suggestion.total) + Number(tmp_additional.total)) * taxRate) | formatPrice }} 
                            </p>
                        </div>
                    </div>
                </div> 
        ...
        <div class="d-none d-lg-block" style="height: 48px;">
            <div id="checkout-button">
                <a v-if="nextStep" @click="nextOrderflowStep" @click.prevent="btnNextDisabled" href="javascript:void(0)" class="button-default button-block nunito-bold font-size-16" :class="btnNextDisabled ? 'button-disabled' : 'button-white-orange' ">Lanjutkan</a>
                <a v-else class="button-default button-block nunito-bold font-size-16 button-disabled" :class="{'as-tooltip' : (flow === 'hosting' && orderflow_step == 1)}" disabled="disabled">
                    Lanjutkan
                    <p v-if="flow === 'hosting' && orderflow_step == 1" class="as-tooltip__info as-tooltip__info--bottom">Silahkan pilih nama Domain Anda.</p>
                </a>
            </div>
        </div>
    </section>
